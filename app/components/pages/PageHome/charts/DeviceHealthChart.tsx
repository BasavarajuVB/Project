...
const DeviceHealthChart: React.FC = () => {
  ...
  const data = [
    { name: 'Maintenance', value: 20, color: '#FF6B35' },
    { name: 'Repeated Failures', value: 25, color: '#1890ff' },
    { name: 'Meter non readable', value: 55, color: '#52c41a' },
  ];
  ...
  const handleClickOutside = React.useCallback((event: MouseEvent) => {
    ...
    const chartElement = document.querySelector(`.${styles.deviceHealthChart}.${styles.expanded}`);
  ...
  return (
    <div 
      className={`${styles.chartWidget} ${styles.deviceHealthChart} ${isExpanded ? styles.expanded : ''}`}
    >
